...
public class ExistentialPreconditionsRequirement extends QuantificationRequirement {

	public static final Requirement EXISTENTIAL_PRECONDITIONS = new ExistentialPreconditionsRequirement();
	
	private ExistentialPreconditionsRequirement(){
		super("existential-preconditions", "existentially quantified preconditions and goals");
	}
	
	@Override
	protected void load(Parser<SExpression> parser){
		super.load(parser);
		PDDLManager.allowGoalType(ExistentialQuantification.class, parser);
	}

	@Override
	public boolean isRequiredBy(Domain domain){
		for(Operator operator : domain.operators)
			if(operator.precondition.toDNF().collect(ExistentialQuantification.class).size() > 0)
				return true;
		for(AxiomTemplate axiom : domain.axioms)
			if(axiom.context.toDNF().collect(ExistentialQuantification.class).size() > 0)
				return true;
		return false;
	}
	...
	public boolean isRequiredBy(Problem problem){
		return problem.goal.toDNF().collect(ExistentialQuantification.class).size() > 0;
	}
}
